...
test.describe("Input Visual Snapshots", () => {
	...
	test("default input", async ({ page }) => {
		const input = page.locator('#component-display input[type="text"]').first();
	...
	test.describe("Input Types", () => {
		test("text input", async ({ page }) => {
			const input = page
				.locator('#component-display input[type="text"]')
				.first();
			await expect(input).toHaveScreenshot("input-text.png");
		});

		test("email input", async ({ page }) => {
			const input = page.locator('input[type="email"][placeholder="Email"]');
		...
		test("number input", async ({ page }) => {
			const input = page.locator('input[type="number"][placeholder="Number"]');
			await expect(input).toHaveScreenshot("input-number.png");
		});

		test("search input", async ({ page }) => {
			const input = page.locator('input[type="search"][placeholder="Search"]');
	...
	test.describe("Input with Labels", () => {
		test("input with label", async ({ page }) => {
			const container = page.locator("div").filter({ hasText: "Name" }).first();
