...
test.describe("Button Visual Snapshots", () => {
	...
	test("default button", async ({ page }) => {
		const button = page.locator("button").filter({ hasText: "Click" }).first();
		await expect(button).toHaveScreenshot("button-default.png");
	});

	test.describe("Button Variants", () => {
		test("primary button", async ({ page }) => {
			const button = page.locator("button").filter({ hasText: "Default" });
		...
		test("ghost button", async ({ page }) => {
			const button = page.locator("button.ghost").filter({ hasText: "Ghost" });
	...
	test.describe("Button States", () => {
		...
		test("loading icon button", async ({ page }) => {
			const button = page.locator('button[aria-busy="true"].icon.destructive');
	...
	test.describe("Buttons with Icons", () => {
		test("button with icon and text", async ({ page }) => {
			const button = page.locator("button").filter({ hasText: "Mark" });
			await expect(button).toHaveScreenshot("button-with-icon-and-text.png");
		});

		test("icon only button", async ({ page }) => {
			const button = page.locator("button.icon.secondary");
